...
%on CRN vs. Non-CRN.  This code implements CRN version
%of SPSA.
...
format long;
p=10;
N=20000;			%total no. of loss measurements (2 x no. of iterations)
cases=50;
alpha =1;
gamma =.49;
a=.7;
...
theta_0=ones(p,1);  
loss='loss_exponential_CRN';          % loss function used in algorithm operations
rand('seed',35111137)
losses=zeros(cases,1);
thetanorm=zeros(cases,1);
for i=1:cases
  theta=theta_0;
  for k=1:N/(2*gavg)
    ak = a/(k+A)^alpha;
    ...
    for j=1:gavg
      ...
      yplusminus=feval(loss,thetaplus,thetaminus);
      ghat = yplusminus./(2*ck*delta)+ghat;
    end
  ...
  % statements below produce loss value for final evaluation
  lossvalue=0;
  for j=1:p
...
losses %statement for use with example 14.9
